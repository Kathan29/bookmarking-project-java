	...
	<div
		...
		<div
			...
			<b> <a href="${pageContext.request.contextPath}/home"
				...
				<a href="${pageContext.request.contextPath}/myWeblink"
				style="font-family: garamond; font-size: 16px; margin-left: 10px; color: white; text-decoration: none;">My
					Weblinks</a> <a href="${pageContext.request.contextPath}/auth/logout"
	...
	<div class="weblink-container">
		<c:forEach var="weblink" items="${weblinks}">
			<div class="weblink-card">
				<img src="${weblink.imageUrl}" class="weblink-image"
					alt="${weblink.title}">

				<div class="weblink-details">
					<h2 class="weblink-title">${weblink.title}</h2>
					<div class="meta-info">
						Url: <strong>${weblink.url}</strong>
					</div>
					<div class="meta-info">
						Host: <span style="color: #f39c12;">${weblink.host}</span>
					</div>
					...
					<c:if test="${userType==1 or userType==2}">
						<div style="font-size: 14px; color: #777;">Kid Friendly
							Eligibility:</div>
						<c:choose>
							<c:when test="${weblink.kidFriendlyStatus=='UNKNOWN'}">
								<a
									href="${pageContext.request.contextPath}/approveWebKF?wid=${weblink.id}"
									class="btn-action" style="background: #27ae60;">Approve</a>
								<a
									href="${pageContext.request.contextPath}/rejectWebKF?wid=${weblink.id}"
									class="btn-action" style="background: #c0392b;">Reject</a>
							</c:when>
							<c:when test="${weblink.kidFriendlyStatus=='APPROVED'}">
					...
					<c:if
						test="${userType==0 and weblink.kidFriendlyStatus=='APPROVED'}">
						<span class="status-badge"
							style="background: #e1f5fe; color: #0277bd;">Kid Friendly</span>
					</c:if>

					<br> <a
						href="${pageContext.request.contextPath}/saveWeblink?wid=${weblink.id}"
						class="btn-action">Save to My weblinks</a>
				</div>
			</div>
		</c:forEach>
	</div>
