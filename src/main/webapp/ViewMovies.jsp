	...
	<div
		...
		<div
			...
			<b> <a href="${pageContext.request.contextPath}/home"
				...
				<a href="${pageContext.request.contextPath}/myMovie"
				style="font-family: garamond; font-size: 16px; margin-left: 10px; color: white; text-decoration: none;">My
					Movies</a> <a href="${pageContext.request.contextPath}/auth/logout"
	...
	<div class="movie-container">
		<c:forEach var="movie" items="${movies}">
			<div class="movie-card">
				<img src="${movie.imageUrl}" class="movie-image"
					alt="${movie.title}">
				...
				<div class="movie-details">
					...
					<div class="meta-info">
						IMDB Rating: <span style="color: #f39c12;">${movie.imdbRating}</span>
					</div>
					<div class="meta-info">Release Year: ${movie.releaseYear}</div>
					...
					<c:if test="${userType==1 or userType==2}">
						<div style="font-size: 14px; color: #777;">Kid Friendly
							Eligibility:</div>
						<c:choose>
							<c:when test="${movie.kidFriendlyStatus=='UNKNOWN'}">
								<a
									href="${pageContext.request.contextPath}/approveMovieKF?mid=${movie.id}"
									class="btn-action" style="background: #27ae60;">Approve</a>
								<a
									href="${pageContext.request.contextPath}/rejectMovieKF?mid=${movie.id}"
									class="btn-action" style="background: #c0392b;">Reject</a>
							</c:when>
							<c:when test="${movie.kidFriendlyStatus=='APPROVED'}">
					...
					<c:if test="${userType==0 and movie.kidFriendlyStatus=='APPROVED'}">
						<span class="status-badge"
							style="background: #e1f5fe; color: #0277bd;">Kid Friendly</span>
					</c:if>

					<br> <a
						href="${pageContext.request.contextPath}/saveMovie?mid=${movie.id}"
						class="btn-action">Save to My movies</a>
				</div>
			</div>
		</c:forEach>
	</div>
